...
import textQueryRequestResponse, {
  generateOrderId,
} from "./utiles/DialogflowHelper.mjs";
import {
  WebhookClient,
  Card,
  Suggestion,
...
app.post("/dialogwebhook", async (req, res) => {
  ...
  function showMenu(agent) {
    ...
    agent.add(new Image("image-menu"));
  ...
  async function order(agent) {
    ...
    if (slots?.dish && slots?.quantity) {
      ...
      agent.add(
        `${slots.dish} for ${slots.quantity} person is added in your cart.`
      );
  ...
  async function coldDrink_yes(agent) {
    ...
    if (slots?.coldDrinkSize && slots?.coldDrinkFlavour) {
      ...
      agent.add(
        `${slots.coldDrinkSize} Size cold drink is added in your cart.`
      );
    ...
    try {
      ...
      if (
        ...
        cart[0]?.baverages?.map((item, index) => {
          agent.add(
            `${index + 1}- Flavour: ${item.beverageFlavour} Size: ${item.size}`
          );
        });
      }
    } catch (error) {
  ...
  async function showOrderStatus(agent) {
    ...
    if (slots?.orderId) {
      ...
      if (order) {
        ...
        order.waitingTime !== "0" &&
          agent.add(
            `Your order will be delivered in ${order.waitingTime} minutes.`
          );
        agent.add("Say show menu if you want to see menu");
        agent.add(new Suggestion("show menu"));
      } else {
